...
<%@page import="com.primus.data.CourseForm"%>
...
<div class="row" id="courseform-arena">
    <% CourseForm courseForm = (CourseForm)request.getAttribute("courseFormInstance");  %>
    <div class="col-12">
        <div class="widget-box">
            <div class="widget-title" style="height: 0px">
                <c:choose>
                    <c:when test="${courseFormInstance eq null and not academicSessionX.courseRegistrationClosed}">
                        ...
                        <c:when test="${courseFormInstance eq null and academicSessionX.courseRegistrationClosed}">
                        <a class="btn btn-success btn-mini" href="/Primus/student/download?academicSession=${academicSessionX.id}&semester=${semesterX}&action=courseform" >Download Course Form</a> 
                    </c:when>
            ...
            <div class="widget-content" style="padding-top:10%;">
                    ...
                    <tbody>
                        <c:forEach items="${courseFormInstance.courses}" var="course" >
                            <tr>
                                ...
                                <c:choose> 
                                    <c:when test="<%= courseForm.getStatus().name().equalsIgnoreCase(Status.APPROVED.name())%>">
                                        <td><c:out value="YES"/></td>
                                    </c:when>
                                    <c:otherwise>
                                ...
                                <c:choose> 
                                    <c:when test="<%= courseForm.getStatus().name().equalsIgnoreCase(Status.APPROVED.name()) || courseForm.getStatus().name().equalsIgnoreCase(Status.IN_PROGRESS.name())%>">
